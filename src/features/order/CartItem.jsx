		...
		<li className='flex w-full items-center justify-between py-4 text-sm'>
			...
			<div className='space-y-2'>
				...
				<p className='space-x-4'>
					...
					<span className='text-sm text-rose-500'>@ {formatCurrency(item.product.price)}</span>
					{!confirmed && <span className={className.base}>{formatCurrency(item.totalPrice)}</span>}
				</p>
			</div>

			{confirmed && <span className={className.confirmed.price}>{formatCurrency(item.totalPrice)}</span>}
