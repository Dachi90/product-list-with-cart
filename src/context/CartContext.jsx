...
function reducer(state, action) {
	switch (action.type) {
		...
		case 'cart/decrease':
			// payload would be an id
			const curItem = state.cart.find((item) => item.product.id === action.payload);

			if (curItem.occurence === 1)
				return {
					...state,

					cart: state.cart.filter((item) => item.product.id !== curItem.product.id),
				};
...
function CartContextProvider({ children }) {
	...
	const totalPrice = isEmpty ? 0 : cart.reduce((acc, cur) => acc + cur.totalPrice, 0);

	const totalQuantity = isEmpty ? 0 : cart.reduce((acc, cur) => acc + cur.occurence, 0);
...
export function useCart() {
	...
	if (context === undefined) throw new Error('Cannot use CartContext outside CartContextProvider');
	return context;
}
